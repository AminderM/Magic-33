    ...
    <div className="space-y-6">
      ...
      {drivers.length > 0 && (
        ...
        <TabsContent value="pending" className="space-y-6 mt-6">
          <div className="mb-4">
            <h3 className="text-lg font-semibold text-gray-900">Pending Driver Applications</h3>
            <p className="text-sm text-gray-600">Owner-operators who have signed up via the Driver App</p>
          </div>
          ...
          {pendingDrivers.length === 0 ? (
            <Card>
              <CardContent className="p-8 text-center">
                ...
                <p className="text-gray-600">
                  Owner-operator driver signups will appear here for your approval
                </p>
              </CardContent>
            </Card>
          ) : (
                      ...
                      <div className="flex items-center gap-4 flex-1">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-4 mt-1 text-sm text-gray-600">
                            <span><i className="fas fa-envelope mr-1"></i>{driver.email}</span>
                            <span><i className="fas fa-phone mr-1"></i>{driver.phone}</span>
                          </div>
        ...
        <TabsContent value="assignments" className="space-y-6 mt-6">
          <div className="flex justify-between items-start mb-4">
            <div>
              <h3 className="text-lg font-semibold text-gray-900">Assign Loads to Drivers</h3>
              <p className="text-sm text-gray-600">Select a driver and assign them a load</p>
            </div>
          </div>
            ...
            <Card>
              <CardContent className="p-8 text-center">
                ...
                <p className="text-gray-600 mb-4">
                  Add drivers to your fleet before assigning loads
                </p>
                <Button onClick={() => setActiveManagementTab('fleet')}>
                  <i className="fas fa-user-plus mr-2"></i>
                  Add Drivers
                </Button>
              </CardContent>
              ...
              {drivers.map((driver) => (
                <Card key={driver.id}>
                  <CardContent className="p-6">
                    ...
                    <Dialog open={showAssignLoadDialog && selectedDriverForLoad?.id === driver.id} onOpenChange={(open) => {
                      if (!open) {
                        setShowAssignLoadDialog(false);
                      ...
                      <DialogContent>
                        <DialogHeader>
                          <DialogTitle>Assign Load to {driver.full_name}</DialogTitle>
                        </DialogHeader>
                        <div className="space-y-4 mt-4">
                          <div className="space-y-2">
                            ...
                            <Select value={selectedLoad} onValueChange={setSelectedLoad}>
                              ...
                              <SelectContent>
                                {loads.filter(load => !load.driver_id || load.driver_id === driver.id).map((load) => (
                                  <SelectItem key={load.id} value={load.id}>
                                    {load.order_number} - {load.pickup_city} to {load.delivery_city}
                                  </SelectItem>
                                ))}
                              </SelectContent>
                            </Select>
                          ...
                          <div className="flex justify-end gap-2 pt-4">
                            <Button variant="outline" onClick={() => setShowAssignLoadDialog(false)}>
                              Cancel
                            </Button>
                            <Button onClick={handleAssignLoad} disabled={!selectedLoad}>
                              Assign Load
                            </Button>
                          </div>
                        </div>
